- Added explicit nested Git source materialization for external corpora with parent gitlink + `.gitmodules` path/URL/SHA verification, independent hook/filter/recursive-submodule-disabled nested checkouts and restore, plus authorized MoonBit `moon.work` workspace prebuild through structured `moon build --target js --release` argv; a pinned Canopy dogfood corpus records its seven nested sources without weakening the passing production gate.
