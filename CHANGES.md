...
- Keep OS-enforced Web builds generic and read-only by allowing only inferred build scratch: Vite `node_modules/.vite-temp`, Cloudflare Vite `.wrangler`, and MoonBit workspace `_build` / `.mooncakes`; strict/constrained campaigns preserve the host `MOON_HOME` toolchain input and pre-bootstrap MoonBit JS dependencies before network-denied execution.
