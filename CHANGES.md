- Prepared the first public CalVer release workflow: prefixless `YYYY.MM.PATCH` tags in `Asia/Tokyo`, explicit Linux x86_64 / macOS Apple Silicon / Windows x86_64 artifact runners, draft-first GitHub Releases with reviewed notes, and a release-gate contract that permanently excludes Intel Mac artifacts.
- Hardened strict Linux private-worktree cleanup for restrictive generated permissions: bubblewrap overlays now allocate a fresh workdir per stage and treat OS-temporary overlay reclamation as best-effort so cleanup cannot replace an otherwise completed campaign result; campaign execution diagnostics also retain the canonical intervention code while exposing the underlying OS error separately.
- Promoted the frontier 7/7 evidence into `promoted-production`: seven exact-revision topology contracts now require 100% auto-onboarding and deterministic replay, zero human intervention, zero adapter LOC, zero regressions, clean checkout restoration, a committed baseline, machine-readable first-release gating, and CI execution alongside the 11-target external production gate.
- Keep OS-enforced Web builds generic without mutating the checkout: strict Linux campaigns use a private persistent overlayfs worktree through unprivileged bubblewrap when available, with a privileged host-mounted overlayfs fallback, so build tools may delete/recreate outputs or emit generated files while host source and `.git` remain unchanged; build-time network is denied first and may receive one credential-safe bootstrap retry only when the failed build proves an external network prerequisite. Vite/Astro/SvelteKit/Docusaurus/React Router scratch inference remains as the compatibility fallback for older backends.
- Reached a 7/7 viable external frontier with zero human intervention, 100% deterministic replay, and zero project-specific adapter LOC by replacing unhealthy pins, preferring conservative installed Node runtimes for open-ended engine ranges, sizing Generic Browser watchdogs by bounded trace-replay work, atomically shallow-materializing exact Git revisions, and isolating stage process groups so successful build/prepare commands cannot leak helper daemons.
- Split frontier target viability from generic capability: campaign schema v2 classifies declared install/workspace-build/project-build/managed-start failures as target viability failures while browser-reached targets are qualified; benchmark schema v3 reports viability distributions, qualified-only generic capability rate, and requires all targets to be viable before production promotion.
- Frontier KPI follow-up: infer package-manager install roots and ancestor package-manager declarations for nested monorepo/PnP projects, topologically prebuild local `workspace:*` dependencies, classify required-stage interventions into build/server/browser/timeout causes, and record the canonical 3/7 frontier baseline while separating broken upstream pins from generic capability gaps.
- Added machine-readable Frontier score KPIs (auto-onboarding, intervention-reason distribution, deterministic replay rate, adapter LOC, promotion eligibility) and bounded dependency preparation with configurable timeout/process-tree cleanup so a stalled install becomes `prepare_timeout` instead of blocking the corpus.
- Added a separate novelty-weighted external frontier corpus with seven pinned OSS repositories spanning SvelteKit, Astro, Remix custom server, Remix + Prisma SSR/DB, Lit Web Components, Yarn Berry PnP monorepo, and legacy Webpack; unknown-Web inspection now recognizes SvelteKit/Astro/Remix/Lit families, `.svelte`/`.astro` sources, and PnP install mode without weakening the production external gate.
- Completed the explicit nested Git source + workspace-prebuild dogfood path: nested declarations may now safely name descendants under another explicitly declared source and are checked against the nearest declared ancestor gitlink/`.gitmodules` entry without recursive acquisition; the pinned Canopy corpus covers 7 top-level + 3 Loom child checkouts, runs `moon build --target js --release`, constrains Waku writes to `dist`/`.wrangler`, and passes the live benchmark with 100% auto-onboarding, 0 interventions, deterministic replay, 0 adapter LOC, and clean post-run restore.
- Expanded the pinned external OSS production corpus to 11 targets across 6 repositories by adding Ensenzu with exact Corepack `pnpm@11.5.3` auto-prepare from a dependency-unprepared checkout; Generic Browser now accepts one bounded unambiguous nested HTML entry such as `dist/client/index.html`, and external benchmark restoration removes newly created ignored roots while preserving pre-existing ignored state.
- Expanded the pinned external OSS production corpus from 3 to 10 targets across 5 repositories with zero project-specific executable adapter LOC, explicit target/repository/tag breadth gates, observed framework/mode/package-manager/state-source distributions, single-HTML static entry onboarding, gitlink-contained target rejection, submodule-independent parent cleanliness checks, and post-benchmark checkout restoration.
- Added explicit pinned external OSS corpus materialization and verification with hook/submodule-disabled Git acquisition, exact origin/revision/cleanliness checks, zero implicit clone/fetch during benchmarking, and post-benchmark checkout restoration.
- Hardened blind Web onboarding from pinned OSS dogfood: canonical package-manager projects may prove readiness from all declared dependencies when manager markers are absent, coverage-guided action execution errors remain diagnostics instead of aborting the stage, and Generic Browser stage timeout now scales deterministically with exploration, replay, and volatility bounds.
- Added a committed production benchmark baseline gate that detects project-level auto-onboarding, replay determinism, and human-intervention regressions while reporting finding deltas independently; CI now runs the live production corpus against the machine-independent baseline.
...
- Added versioned machine-readable domain projection/property hint contracts: human approval now pins deterministic contracts, runtime coverage is granted only to supported executable contracts, unsupported or legacy missing contracts remain explicit `domain_unverified` diagnostics, and Generic Browser campaigns without hints are unchanged.
- Added a machine-readable sandbox capability model across filesystem/network/process with `strict`, `constrained`, and `caller_enforced` levels; strict runs now fail before stage execution unless every required axis is strict, and Linux bubblewrap adds PID isolation to the existing filesystem/network boundary.
- Added an explicit Generic/domain semantic oracle boundary to Generic Browser reports: generic checks and replay can pass while domain correctness remains `domain_unverified`; only human-approved executable properties can produce domain verification, while projections remain observational and unsupported hints never count as verified coverage.
...
- Added a Linux bubblewrap strict Web execution sandbox with outbound network isolation, PID namespace/new-session process isolation, read-only source and `.git`, explicit build/artifact writable paths, private tmpfs, credential environment filtering, machine-readable capability reporting, runner integration, and a live CI escape-policy probe.
...
- Extended drawDB dogfood with DBML/MySQL semantic round-trips, real UI import/export coverage, and repeated drag undo/redo coordinate checks; detected a production-browser valid-MySQL import failure while confirming zero drag position drift.
