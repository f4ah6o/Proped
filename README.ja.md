...
Frontier benchmark summaryは`frontierScore`に加え、aggregateの`interventionReasonDistribution`、`deterministicReplayRate`、`projectSpecificAdapterLoc`を出力します。`frontierScore.promotionEligible`は7/7 auto-onboarding、全targetでreplay観測かつdeterministic、adapter LOC 0のすべてを満たした場合だけtrueになります。dependency bootstrapはboundedで、`--prepare-timeout-ms <ms>`により既定300秒のinstall timeoutを上書きできます。timeoutしたprocess treeは残りのfrontierを停止させず、明示的な`prepare_timeout` intervention classになります。
