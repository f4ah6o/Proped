...
レポートには、生成時の操作列ではなく、縮約後の操作列が含まれます。

### ブラウザを使わない Rabbita レンダリング

`rabbita_machine` は Rabbita のサーバーサイドレンダラーを使います。
...
| `fingerprint` | 状態の安定した識別子と重複排除キー |
| `describe_msg` | 人間が読めるトレース項目 |
...
- `run`：状態を探索し、プロパティを評価し、レポートを生成する
