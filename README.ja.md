...
`--corpus frontier`（alias: `novelty`）は`protocol/fixtures/external-frontier-corpus.json`に固定した、target数ではなくproject topologyの未知性を優先する別corpusです。SvelteKit、Astro、custom Express serverを持つRemix、Remix + PrismaのSSR/database state、Lit Web Components、Yarn Berry PnP monorepo workspace、legacy Webpackという7つの異なるOSS repositoryをfull commit SHAでpinします。structural gateは7 repositoryすべて、project-specific executable adapter LOC = 0、および`sveltekit` / `astro` / `remix` / `web-components` / `legacy-webpack` / `monorepo` / `custom-server` / `ssr-db` / `pnp`のnovelty tag coverageを要求します。このcorpusではauto-onboarding failureを許容し、未対応shapeを観測可能なまま`external` production gateの合格条件を弱めません。
...
Frontier benchmark summaryは`frontierScore`に加え、aggregateの`interventionReasonDistribution`、`viabilityDistribution`、`viabilityFailureDistribution`、`deterministicReplayRate`、`projectSpecificAdapterLoc`を出力します。各targetの`viability`は、declared dependency install / workspace build / project build / managed start自体が成立しない場合を`failed`、browser lifecycleまで到達した場合を`qualified`として、upstream/pin viabilityとProped generic capability failureを分離します。`frontierScore.genericCapability`はqualified targetだけのauto-onboarding率を示します。`frontierScore.promotionEligible`は7/7 auto-onboarding、全targetでreplay観測かつdeterministic、adapter LOC 0のすべてを満たした場合だけtrueになります。dependency bootstrapはboundedで、`--prepare-timeout-ms <ms>`により既定300秒のinstall timeoutを上書きできます。timeoutしたprocess treeは残りのfrontierを停止させず、明示的な`prepare_timeout` intervention classになります。`packageManager`宣言のないpnpm projectでは、runtime世代を一意に決められる古いlockfile format（`5.4` / `6.0`）をhostのpnpmではなくpinしたCorepack proxyで実行します。Corepack auto-pinも無効化するため、prepareがupstream `package.json`へ`packageManager`を追記することはありません。nested JavaScript workspaceではancestorのexact `packageManager`を継承し、lockfile/PnPのinstall rootを基準にdependency readinessを判定し、target build前に明示的なlocal `workspace:*` dependencyを依存順にprebuildできます。
