...
`--corpus frontier` (alias: `novelty`) uses `protocol/fixtures/external-frontier-corpus.json`, a separate topology-frontier corpus selected for unfamiliar project shape rather than target volume. It pins 7 distinct OSS repositories spanning SvelteKit, Astro, Remix with a custom Express server, Remix + Prisma SSR/database state, Lit Web Components, a Yarn Berry PnP monorepo workspace, and legacy Webpack. Its structural gate requires all 7 repositories, zero project-specific executable adapter LOC, and coverage of the novelty tags `sveltekit`, `astro`, `remix`, `web-components`, `legacy-webpack`, `monorepo`, `custom-server`, `ssr-db`, and `pnp`. Auto-onboarding failures are intentionally allowed in this corpus so unsupported shapes remain measurable without weakening the passing `external` production gate.
...
Frontier benchmark summaries expose `frontierScore` plus aggregate `interventionReasonDistribution`, `deterministicReplayRate`, and `projectSpecificAdapterLoc`. `frontierScore.promotionEligible` becomes true only at 7/7 auto-onboarding, replay observed and deterministic for every target, and adapter LOC 0. Dependency bootstrap is bounded; `--prepare-timeout-ms <ms>` overrides the default 300-second install timeout and timed-out process trees become the explicit `prepare_timeout` intervention class instead of blocking the remaining frontier. For pnpm projects without a `packageManager` declaration, historical lockfile formats with an unambiguous runtime generation (`5.4` and `6.0`) are executed through pinned Corepack proxies instead of the host pnpm; Corepack auto-pin is disabled so preparation never writes a `packageManager` field upstream. Nested JavaScript workspaces inherit an exact ancestor `packageManager`, resolve dependency readiness at the lockfile/PnP install root, and may prebuild explicit local `workspace:*` dependencies in dependency order before the target build.
