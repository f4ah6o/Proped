...
The external campaign currently includes eleven targets. `mooncakes-official-ui` explores 780 states and 4,856 transitions across the production Build Queue, official website state, and full-stack tutorial. It retains a three-action stale Build Queue response and a five-action late tutorial reply. `moonclaw-job` shows that an older same-run snapshot can replace a newer terminal snapshot, while `rabbita-xterm-lifecycle` shrinks invalid dimensions to `Resize(cols=0, rows=24)`.
