...
The external campaign currently includes twelve targets. `selene-editor-assets` explores 920 states and 2,098 transitions, retaining duplicate initialization as `Initialize -> Initialize` and a reverse-ordered asset-list response as a second failure. Entity selection normalization for deleted or missing preview IDs passes. `mooncakes-official-ui` continues to retain its Build Queue and tutorial response-order failures.
