...
- Managed Playwright engine selection is generic and additive. Chromium remains the default with its existing metadata shape; WebKit is explicit/optional, can be installed reproducibly with `npm run install:webkit` from `web/playwright-browser`, and reports `managed_webkit_launch_failed` when its executable is unavailable. The opaque replay test executes the same ordinal trace under WebKit whenever that runtime is installed.
- `observeOpaqueWebReplayV1` plus `scripts/web_observe_url_opaque.mjs` replay an existing portable `kind + ordinal` vector under another managed engine and record that engine's fixed transition classes across fresh contexts. Cross-engine observations are explicitly `not-one-minimal`; they do not inherit the source engine's minimization claim. The CLI accepts the opaque replay on stdin and keeps raw loopback URLs/content out of output and failure evidence.
