...
- `external run all` はpure、subscription-model、effect-modelの6 targetを同一CLI契約で実行する。
...
- `dowdiness/incr`のtyped spreadsheetをpure adapterとしてdogfoodし、exact worksheet trace、Eq backdating、no-backdate semanticsとformula overflowを再現した。
