...
function usage(message) {
  ...
  else console.log("Usage: node scripts/web_project_benchmark.mjs <project> [project ...] [--corpus <production|file>] [--previous <summary.json>] [--baseline <baseline.json>] [--no-prepare] [--offline] [--no-artifacts] [--project-artifacts] [--output <dir>] [--sandbox-mode <auto|manifest|strict|constrained|caller-enforced>]\n\nDirect paths require all targets to auto-onboard. Corpus mode evaluates its versioned production quality gate, keeps findings separate, and can compare either a prior full summary (--previous) or a committed stable baseline (--baseline).");
...
for (let index = 0; index < argv.length; index += 1) {
  ...
  else if (arg === "--output" || arg === "--sandbox-mode" || arg === "--corpus" || arg === "--previous" || arg === "--baseline") {
...
try {
  const options = { prepare, offline, writeArtifacts, projectArtifacts, output, sandboxMode, previous, baseline };
  const result = corpus
