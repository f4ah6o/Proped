...
import { VueComponentDriver } from "./vue-component-driver.mjs";
...
const FIXTURE = path.join(
  ROOT,
  "protocol/fixtures/vue-component-mode-result.json",
);
...
async function replayForViolation(driver, trace, code) {
  await driver.reset(7, "vue-fault-form");
  let found = null;
...
async function verifyKnownFailures(driver) {
  ...
  const initialSnapshot = driver.snapshot();
  assert.equal(initialSnapshot.applicationState.suspenseReady, true);
  assert.equal(initialSnapshot.applicationState.teleportMounted, true);
  assert.equal(initialSnapshot.applicationState.piniaTitle, "");
  ...
  const typeTitle = findAction(
    initialActions,
    (action) => action.kind === "type" && action.target.name === "Title" && action.input === "a",
    "type Pinia title",
  );
  const titleResult = await driver.execute(typeTitle);
  assert.equal(titleResult.snapshot.applicationState.piniaTitle, "a");

  await driver.reset(7, "vue-fault-form");
  await driver.execute(await findActionById(driver, typeSearchA.id));
  await driver.execute(await findActionById(driver, typeSearchAB.id));
  ...
  return {
    ...
    boundaries: {
      nextTick: true,
      suspense: initialSnapshot.applicationState.suspenseReady,
      teleport: initialSnapshot.applicationState.teleportMounted,
      pinia: titleResult.snapshot.applicationState.piniaTitle === "a",
    },
...
async function runBenchmark(driver, transitions = 10_000) {
  await driver.reset(131, "vue-fault-form");
  ...
  assert.ok(elapsedMs < BENCHMARK_MAX_MS, `Vue benchmark exceeded ${BENCHMARK_MAX_MS}ms: ${elapsedMs}ms`);
  return {
    transitions,
...
function writeArtifacts(result) {
  ...
  fs.writeFileSync(
    ...
    "digraph vue_component { initial -> stale [label=\"type/type/deliver\"]; initial -> duplicate [label=\"submit/submit\"]; initial -> invalid [label=\"type invalid\"]; }\n",
  );
  fs.writeFileSync(
    path.join(OUTPUT, "atlas.svg"),
    `<svg xmlns="http://www.w3.org/2000/svg" width="720" height="180"><rect width="100%" height="100%" fill="white"/><text x="24" y="38">Vue Component Mode</text><text x="24" y="72">${result.benchmark.transitions} transitions / ${result.benchmark.distinctStates} states</text><text x="24" y="106">${result.failures.map((failure) => failure.property).join(" · ")}</text></svg>\n`,
  );
  fs.writeFileSync(
    path.join(OUTPUT, "atlas.html"),
    `<!doctype html><html><meta charset="utf-8"><title>Vue Component Atlas</title><body><h1>Vue Component Mode</h1><p>${result.benchmark.transitions} transitions</p><pre>${JSON.stringify(result.failures, null, 2).replaceAll("&", "&amp;").replaceAll("<", "&lt;")}</pre></body></html>\n`,
  );
}

const driver = new VueComponentDriver();
try {
  const known = BENCHMARK_ONLY
    ? { actionCount: 0, corpus: [], boundaries: {}, failures: [], replayHash: "" }
