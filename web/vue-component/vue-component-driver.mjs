...
export class VueComponentDriver {
  ...
  installGlobals() {
    ...
    for (const key of [
      ...
      "HTMLSelectElement", "SVGElement", "Element", "Event", "MouseEvent", "Node",
    ]) {
  ...
  async reset(seed = 1, fixture = "vue-fault-form") {
    if (!this.dom) {
      this.dom = new JSDOM(
        "<!doctype html><html><body><div id=\"app\"></div><div id=\"teleport\"></div></body></html>",
        { url: "http://localhost/vue-form?request=req-1" },
      );
      this.installGlobals();
      this.vue = await import("vue");
      this.piniaModule = await import("pinia");
      this.fixtureModule = await import("./vue-fixture.mjs");
      this.nextTick = this.vue.nextTick;
    } else if (this.app) {
      this.app.unmount();
      this.dom.window.document.body.innerHTML = '<div id="app"></div><div id="teleport"></div>';
    }
    this.seed = seed;
    ...
    this.latestState = null;
    this.consoleEntries = [];
    this.pinia = this.piniaModule.createPinia();
    this.app = this.vue.createApp(this.fixtureModule.FaultyVueForm, {
      onState: (state) => { this.latestState = state; },
    });
    this.app.config.warnHandler = (message) => {
      if (!message.includes("<Suspense> is an experimental feature")) {
        this.consoleEntries.push({ kind: "framework-warning", message });
      }
    };
    this.app.use(this.pinia);
    this.vm = this.app.mount(this.dom.window.document.getElementById("app"));
    await this.settle();
    if (!this.latestState) this.latestState = this.vm.state();
  ...
  roleOf(element) {
    ...
    if (tag === "input") return ROLE_BY_INPUT_TYPE[element.type] ?? "textbox";
  ...
  accessibleName(element) {
    ...
    if (element.id) {
      const label = [...this.dom.window.document.querySelectorAll("label")]
        .find((candidate) => candidate.htmlFor === element.id);
      if (label) return normalizeText(label.textContent);
    }
    if (["BUTTON", "A"].includes(element.tagName)) return normalizeText(element.textContent);
  ...
  scopeOf(element) {
    ...
    if (dialog && dialog !== element) scopes.push(`dialog:${this.accessibleName(dialog)}`);
    const form = element.closest("form");
    if (form && form !== element) scopes.push(`form:${this.accessibleName(form)}`);
    return scopes;
  }
  ...
  discoverDomElements() {
    return [...this.dom.window.document.querySelectorAll("button,a[href],input,select,form,dialog")]
  ...
  matchesTarget(element, target) {
    ...
    return descriptor.role === target.role && descriptor.name === target.name &&
  ...
  findElement(target) {
    const matches = [...this.dom.window.document.querySelectorAll("button,a[href],input,select,form,dialog")]
  ...
  async execute(action) {
    if (this.disposed) throw new Error("Vue component session is disposed");
    const before = this.snapshot();
    if (action.kind === "inject") {
      if (action.target.name === "Search response") this.vm.deliverSearch(action.input.id);
      else if (action.target.name === "Submit response") this.vm.completeSubmit(action.input.id);
    } else {
      ...
      if (action.kind === "type") this.setInputValue(element, action.input);
        ...
        element.dispatchEvent(new this.dom.window.Event("submit", { bubbles: true, cancelable: true }));
      ...
      else throw new Error(`unsupported Vue component action: ${action.kind}`);
    }
    await this.settle();
    ...
    return {
      ...
      settle: { status: "settled", microtasks: 2, timers: 0, renders: 1, elapsedMs: 0 },
  ...
  semanticNode(element) {
    ...
    const children = [...element.childNodes].map((child) => this.semanticNode(child)).filter(Boolean);
    return {
  ...
  snapshot() {
    ...
    const forms = controls.map((element) => ({
      identity: { role: this.roleOf(element), name: this.accessibleName(element), within: this.scopeOf(element) },
      value: element.value,
    ...
    const pending = [
      ...
      ...this.latestState.pendingSubmit.map((request) => ({
        kind: "submit", key: `submit:${request.title}`, generation: request.id,
      })),
    ];
    const active = document.activeElement;
    const focus = active && active !== document.body
      ? { role: this.roleOf(active), name: this.accessibleName(active), within: this.scopeOf(active) }
      : undefined;
    const main = this.semanticNode(document.querySelector("main"));
    const teleport = this.semanticNode(document.querySelector("#teleport"));
    return createSemanticSnapshot({
      url: this.dom.window.location.href,
      semanticDom: { role: "document", name: "Vue fixture", children: [main, teleport].filter(Boolean) },
      forms,
      focus,
      ...
      applicationState: {
        ...
        suspenseReady: document.querySelector('[aria-label="Suspense status"]')?.textContent === "ready",
        teleportMounted: Boolean(document.querySelector('#teleport [aria-label="Teleport boundary"]')),
        piniaTitle: document.querySelector("[data-pinia-title]")?.getAttribute("data-pinia-title") ?? "",
      },
    });
  }
